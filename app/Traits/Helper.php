...
trait Helper
{
    public function responseJson($msg,$data=null,$status=200){
        $res=[
            'message'=>$msg,
            'data'=>$data,
            'status'=>$status
    ...
    public function UploadImage(Request $request,$image,$module,$folder='uploads'){
        ...
        $img_name = random_int(11111111,99999999).".".$img->getClientOriginalExtension();
        $img->move(public_path("$folder"), $img_name);
        $img_path = $folder.'/'.$img_name;
        $module->$image=$img_path;
        $module->save();

    }

    // store and send notification function
    public function notifyByFirebase($title,$body,$tokens,$data = []){        // paramete 5 =>>>> $type
